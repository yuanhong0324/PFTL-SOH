...
class RAKAN_DANN(nn.Module):
    def __init__(self, feature_size=32, input_dim=4, extract_dim=64):
        ...
        self.domain_classifier = nn.Sequential(nn.Linear(16 * 16, 16), nn.ReLU(), nn.Linear(16, 1), nn.Sigmoid())
    ...
    def forward(self, x, is_MOON=False,is_visual_attn=False,is_visual_pre=False):
        ...
        x = self.cnn(x)
        # mmm_out_1 = x.reshape(x.size(0), -1)
        x = self.pool(x)
        x = x.transpose(1, 2)
        ...
        reg_out = self.predictor(out_pre)
        reverse_feature = grad_reverse(out_pre, 1.0)
        ...
        return reg_out, attn, domain_output
